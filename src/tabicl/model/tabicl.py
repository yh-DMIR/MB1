...
from .mb_injection import (
    MBAttentionBias,
    MBEmbeddingAdapter,
    MBHardSelector,
    build_cls_hard_mask_bias,
    build_summary_token,
)
...
class TabICL(nn.Module):
    ...
    def __init__(
        ...
        mb_injection: str = "none",
        mb_bias_init: float = 0.0,
        mb_bias_trainable: bool = True,
        mb_bias_per_layer: bool = True,
        mb_bias_per_head: bool = True,
        mb_bias_target: str = "cls_to_feature",
        mb_hard_mask_threshold: float = 0.5,
        mb_topk_extra: int = 0,
        mb_hard_select_mode: str = "threshold",
        mb_hard_select_topk: int = 0,
        mb_embedding_dim: Optional[int] = None,
        mb_embedding_add_position: str = "before_rowinteraction",
    ):
        super().__init__()
        ...
        self.mb_injection = mb_injection
        self.mb_bias_target = mb_bias_target
        self.mb_hard_mask_threshold = mb_hard_mask_threshold
        self.mb_topk_extra = mb_topk_extra
        self.mb_hard_select_mode = mb_hard_select_mode
        self.mb_hard_select_topk = mb_hard_select_topk
        self.mb_embedding_add_position = mb_embedding_add_position
        ...
        self.mb_attention_bias = None
        self.mb_embedding_adapter = None
        self.mb_hard_selector = None

        if mb_injection in {"cls_soft_bias", "cls_hard_mask", "hard_select"}:
            self.mb_attention_bias = MBAttentionBias(
                num_layers=row_num_blocks,
                num_heads=row_nhead,
                num_cls=row_num_cls,
                bias_init=mb_bias_init,
                trainable=mb_bias_trainable,
                per_layer=mb_bias_per_layer,
                per_head=mb_bias_per_head,
                target=mb_bias_target,
            )
        if mb_injection == "embedding_add":
            self.mb_embedding_adapter = MBEmbeddingAdapter(
                embed_dim=embed_dim,
                hidden_dim=mb_embedding_dim or embed_dim,
            )
        if mb_injection in {"cls_hard_mask", "hard_select"}:
            self.mb_hard_selector = MBHardSelector(
                threshold=mb_hard_mask_threshold,
                mode=mb_hard_select_mode,
                topk=mb_hard_select_topk,
                topk_extra=mb_topk_extra,
            )

    def _prepare_mb_row_inputs(
        self,
        embeddings: Tensor,
        mb_scores: Optional[Tensor] = None,
        d: Optional[Tensor] = None,
    ) -> tuple[Tensor, Optional[Tensor], Optional[list[Tensor]], dict]:
        """Prepare row-interaction inputs for MB-aware ablations."""

        diagnostics = {}
        row_d = d
        row_attn_mask = None

        if mb_scores is None or self.mb_injection == "none":
            return embeddings, row_d, row_attn_mask, diagnostics

        if mb_scores.shape[1] != embeddings.shape[2] - self.row_num_cls:
            raise ValueError(
                f"mb_scores feature dim {mb_scores.shape[1]} does not match embeddings feature dim "
                f"{embeddings.shape[2] - self.row_num_cls}"
            )

        num_rows = embeddings.shape[1]
        feature_embeddings = embeddings[:, :, self.row_num_cls :, :]

        if self.mb_injection == "embedding_add":
            feature_embeddings = self.mb_embedding_adapter(feature_embeddings, mb_scores)
            embeddings = embeddings.clone()
            embeddings[:, :, self.row_num_cls :, :] = feature_embeddings
            diagnostics["embedding_add_mean"] = feature_embeddings.mean().detach()
            return embeddings, row_d, row_attn_mask, diagnostics

        if self.mb_injection == "summary_token":
            summary_token = build_summary_token(feature_embeddings, mb_scores)
            embeddings = torch.cat([embeddings[:, :, : self.row_num_cls, :], summary_token, feature_embeddings], dim=2)
            row_d = row_d + 1 if row_d is not None else row_d
            diagnostics["summary_token_norm"] = summary_token.norm(dim=-1).mean().detach()
            return embeddings, row_d, row_attn_mask, diagnostics

        if self.mb_injection == "cls_soft_bias":
            row_attn_mask = [
                bias.unsqueeze(1).expand(-1, num_rows, -1, -1, -1)
                for bias in self.mb_attention_bias(mb_scores, embeddings.shape[2])
            ]
            diagnostics["mb_score_mean"] = mb_scores.mean().detach()
            return embeddings, row_d, row_attn_mask, diagnostics

        if self.mb_injection == "cls_hard_mask":
            row_attn_mask = [
                bias.unsqueeze(1).expand(-1, num_rows, -1, -1, -1)
                for bias in build_cls_hard_mask_bias(
                    mb_scores,
                    num_layers=self.row_num_blocks,
                    num_heads=self.row_nhead,
                    num_cls=self.row_num_cls,
                    threshold=self.mb_hard_mask_threshold,
                    topk_extra=self.mb_topk_extra,
                )
            ]
            diagnostics["masked_features"] = (mb_scores < self.mb_hard_mask_threshold).float().mean().detach()
            return embeddings, row_d, row_attn_mask, diagnostics

        if self.mb_injection == "hard_select":
            selected_mask = self.mb_hard_selector.get_mask(mb_scores)
            feature_embeddings = feature_embeddings * selected_mask.unsqueeze(1).unsqueeze(-1).to(feature_embeddings.dtype)
            embeddings = embeddings.clone()
            embeddings[:, :, self.row_num_cls :, :] = feature_embeddings
            row_attn_mask = [
                bias.unsqueeze(1).expand(-1, num_rows, -1, -1, -1)
            ...
            diagnostics["selected_ratio"] = selected_mask.float().mean().detach()
            return embeddings, row_d, row_attn_mask, diagnostics
    ...
    def _train_forward(
        ...
        mb_scores: Optional[Tensor] = None,
        return_mb_diagnostics: bool = False,
        ...
        embeddings = self.col_embedder(X, d=d, train_size=None if embed_with_test else train_size)
        embeddings, row_d, row_attn_mask, diagnostics = self._prepare_mb_row_inputs(embeddings, mb_scores=mb_scores, d=d)
        representations = self.row_interactor(embeddings, d=row_d, attn_mask=row_attn_mask)

        # Dataset-wise in-context learning
        out = self.icl_predictor(representations, y_train=y_train)

        if return_mb_diagnostics:
    ...
    def _inference_forward(
        ...
        embeddings, row_d, row_attn_mask, diagnostics = self._prepare_mb_row_inputs(embeddings, mb_scores=mb_scores, d=None)
        representations = self.row_interactor(
            embeddings,
            d=row_d,
            mgr_config=inference_config.ROW_CONFIG,
            attn_mask=row_attn_mask,
        )
    ...
    def forward(
        ...
        if self.training:
            out = self._train_forward(
                ...
                mb_scores=mb_scores,
                return_mb_diagnostics=return_mb_diagnostics,
            )
        else:
