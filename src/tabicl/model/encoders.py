...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        attn_mask: Optional[Tensor | int | Sequence[Tensor | int]] = None,
    ) -> Tensor:
        ...
        if isinstance(attn_mask, (list, tuple)):
            if len(attn_mask) != len(self.blocks):
                raise ValueError(f"Expected {len(self.blocks)} attention masks, got {len(attn_mask)}")
            block_masks = attn_mask
        else:
            block_masks = [attn_mask] * len(self.blocks)

        for block, block_mask in zip(self.blocks, block_masks):
            out = block(q=out, key_padding_mask=key_padding_mask, attn_mask=block_mask, rope=self.rope)
