...
class RowInteraction(nn.Module):
    ...
    def _aggregate_embeddings(
        ...
        attn_mask: Optional[Tensor | int | list[Tensor | int]] = None,
    ) -> Tensor:
        ...
        outputs = self.tf_row(embeddings, key_padding_mask=key_mask, attn_mask=attn_mask)  # (B, T, H+C, E)
    ...
    def _train_forward(
        ...
        representations = self._aggregate_embeddings(embeddings, key_mask, attn_mask=attn_mask)  # (B, T, C*E)
    ...
    def _inference_forward(
        ...
        # MB attention bias is batch-specific and may be different for every layer/head.
        # 为了保持最小侵入，这里在存在 attn_mask 时直接走前向，不再做自动切 batch。
        if attn_mask is not None:
            representations = self._aggregate_embeddings(embeddings, attn_mask=attn_mask)
        else:
    ...
    def forward(
        ...
        if self.training:
            representations = self._train_forward(embeddings, d, attn_mask=attn_mask)
        else:
            representations = self._inference_forward(embeddings, mgr_config, attn_mask=attn_mask)
